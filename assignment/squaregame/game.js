...
function timeStart() {
	...
	function timeCount() {
		...
		if(allbox.length <= 0){ // All box are clear >> timer stop and call clear screen function
			alert(second + "เจ้าอยู่สูงกว่าข้า");
			clearScreen();
			clearInterval(timer);
			timer = null;
...
function bindBox(box) {
	let boxContainer = document.getElementById('squares-layer'); // Click to Destroy Box
...
function clearScreen(){
	var allbox = document.querySelectorAll("#squares-layer div");
	var Conbox = document.getElementById('squares-layer');			

	//delete all  squares
	for (var i = 0; i < allbox.length; i++){
		Conbox.removeChild(allbox[i]);
	}
}